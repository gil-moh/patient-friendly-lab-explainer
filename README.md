...
Sample narratives generated by the engine are in [`demo_outputs/`](demo_outputs/):

- [`patient-lab-a1c.txt`](demo_outputs/patient-lab-a1c.txt)
- [`patient-lab-lipids.txt`](demo_outputs/patient-lab-lipids.txt)
- [`patient-lab-cmp.txt`](demo_outputs/patient-lab-cmp.txt)
- [`patient-lab-example-all-context.txt`](demo_outputs/patient-lab-example-all-context.txt)
...
3. Build and start:
   ...
   docker compose up -d
   ```
4. Wait ~60 seconds for IRIS to become healthy, then run a synthetic demo (no FHIR patient data needed):
   ```
...
|---|---|---|
| `FHIR_BASE_URL` | `http://localhost:52773/fhir/r4` | FHIR R4 endpoint for live patient queries |
| `FHIR_BASIC_USER` | _(none)_ | Basic auth username |
| `FHIR_BASIC_PASS` | _(none)_ | Basic auth password |
